...
import { getLogsRequest, submitTestRequest } from '../redux/slices/testSlice';
import { logout } from '../redux/slices/authSlice';

const EventLogs = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { attemptId, logs, loading, testSubmitted } = useSelector((state) => state.test);
  ...
  useEffect(() => {
    if (testSubmitted) {
      ...
      setTimeout(() => {
        ...
        navigate('/login', {
          state: { message: 'Test submitted successfully! You can login again to take another test.' }
        });
      }, 2000);
    }
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-yellow-800">
                if events Logs queued, will be sent in next batch (every 15 seconds)
              </p>
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        <h3 className={`font-medium ${log.is_violation ? 'text-red-900' : 'text-gray-900'}`}>
                          {formatEventType(log.event_type)}
                        </h3>
                        <p className="text-sm text-gray-600 mt-1">{formatTimestamp(log.timestamp)}</p>
                        {log.event_data?.message && (
                          <p className="text-sm text-gray-700 mt-2">{log.event_data.message}</p>
                        )}
                      </div>
                    </div>
                    {log.is_violation && (
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-base text-gray-600">
                  Total violations: <span className="font-semibold text-red-600">{violationCount}</span>
                </p>
              </div>
              <button
